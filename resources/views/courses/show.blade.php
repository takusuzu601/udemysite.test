        ...
        <div class="order-1 lg:order-2">
            <section class="card mb-4">
                <div class="card-body">
                    ...
                    @can('enrolled',$course)
                        <a href="{{route('courses.status',$course)}}" class="btn btn-danger btn-block mt-4">コースを続行します</a>
                    @else

                    @if($course->price->value==0)
                    <p class="text-2xl font-bold text-gray-500 mt-3 mb-2">無料</p>
                    <form action="{{route('courses.enrolled',$course)}}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-danger btn-block">このコースを受講する</button>
                    </form>
                    @else
                    <p class="text-2xl font-bold text-gray-500 mt-3 mb-2">US${{$course->price->value}}</p>
                        <a href="{{route('payment.checkout',$course)}}" class="btn btn-danger btn-block">このコースを購入</a>
